...
export default function ProjectsClosed() {
    ...
    useEffect( async() => {
        ...
        casperProjects.forEach(async (project) => {
            ...
            const status = await casperpadClient.getDataByFieldName(projectUref, "status");
                ...
                <section className="projects">
                    {
                        projects.map((project, index) => {
                            return (
                                ...
                                || (project.status === 'Closed' && (index === 0 && (verifiedPrivate || isAdmin)) && <CustomCard key={index} project={project} />)
                                || ((index === 1 && (verifiedSeed || isAdmin)) && <CustomCardSeed key={index} project={project} />)
                                || ((index === 3 && (verifiedAdvisory || isAdmin)) && <CustomCardAdvisory key={index} project={project} />)
                    ...
                    }
                    {
                        csprProjects.map((project, index) => {
                            return (
                                <CustomCardCASPER key={index} project={project} status={'Closed'} />
                            )
                        })
                    }
                </section>
