...
export default function CustomCardNew({ project }) {
  ...
  useEffect(() => {
    setSoldAmount(
      soldAmount_tmp
        ? Number((soldAmount_tmp / 10 ** 18).toString()).toFixed(2)
        : 0
    );
    setSoldAmount(
      soldAmount_tmp
        ? Number((soldAmount_tmp - tmpSoldAmount) / 10 ** 18).toFixed(5)
        : 0
    );
    setTotalPresaleAmount(
      totalPresaleAmount_tmp
        ? Number((totalPresaleAmount_tmp / 10 ** 18).toString()).toFixed(2)
        : 0
    );
    setProgressValue(
      Number((soldAmount * 100) / totalPresaleAmount).toFixed(2)
    );
    return () => {
