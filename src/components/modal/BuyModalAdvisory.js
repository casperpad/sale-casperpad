...
const BuyModalAdvisory = ({ isOpenBuy, setIsOpenBuy, onlyOneToast }) => {
  ...
  useEffect(() => {
    setBuySwprAmount(Number(buyUsdAmount / 0.008).toFixed(3));
    setTier(maxAmountOfTier ? Number(maxAmountOfTier * 0.008).toFixed(3) : 0);
  ...
  const {
    state: stateApproveUsdt,
    send: approveUsdt,
    events: getEventApproveUsdt,
  } = useUsdtContractMethod("approve");
  const {
    state: stateApproveBusd,
    send: approveBusd,
    events: getEventApproveBusd,
  } = useBusdContractMethod("approve");
  ...
  useEffect(() => {
    if (
      stateApproveUsdt.status == "Success" ||
      stateApproveBusd.status == "Success"
    ) {
      handleClose();
      setIsOpenBuy(true);
  ...
  } = useVestingContractMethod("addVest");
  function handleBuy() {
    if (verified) {
                ...
                <>
                  ...
                  >
                    <div className="text-white m-auto">
                      {" "}
                      The Buyable Maximum Amount Of Your Tier Is ${tier}! <br />
                      (Note: You can insert the token to the vesting schedule
                      with only one buying.)
                    </div>
                  </div>
                  <div
                    data-bs-dismiss="modal"
                    ...
                    <div className="text-white m-auto">
                      <Form>
                        <Form.Check
                          inline
                          defaultChecked={
                            payCurrency == usdtTokenAddress ? true : false
                          }
                          label="USDC"
                          name="group1"
                          type="radio"
                        ...
                        <Form.Check
                          inline
                          defaultChecked={
                            payCurrency == busdTokenAddress ? true : false
                          }
                          label="BUSD"
                          name="group1"
                          type="radio"
...
export default BuyModalAdvisory;
