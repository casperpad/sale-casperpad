...
const MyModal = ({ isOpen, setIsOpen, onlyOneToast}) => {
  ...
  const {
    ...
    async function handleConnectWalletCasper() {
      const address = await Signer.getActivePublicKey().catch(
        (error) => {
          ...
          <div className="outer bg-black absolute top-0 left-0 h-full w-full z-20 opacity-80"></div>
            ...
            <div className="absolute top-0 left-0 h-full w-full z-30 flex items-center justify-center" onClick={() => handleClose()} >
                <div className="inner max-w-screen-sm flex-grow  text-white  bg-gradient-to-br from-yellow-200 to-yellow-700 p-1 opacity-100 rounded-3xl" onClick={ (e) => { e.stopPropagation(); }} >
                    {(binanceConnected && (
                        <>
                            <div data-bs-dismiss="modal" id="wallet-connect-metamask" className="c-list border-b px-3 py-2 d-flex align-items-center">
                                <div className="text-white col-12 m-auto"> <span style={{wordBreak: 'break-all'}}>{binanceAddress}</span></div>
                            </div>
                            <div data-bs-dismiss="modal" id="wallet-connect-metamask" className="c-list border-b px-3 py-2 d-flex align-items-center cursor-pointer">
                                <a href={"https://bscscan.com/address/" + binanceAddress} target="_blank" className="text-white m-auto" rel="noreferrer"> View on BSCScan </a>
                                <a className="text-white m-auto" onClick={ handleDisconnectWalletMeta }> Disconnect</a>
                            </div>
                        </>
                    )) || (
                        <>
                    ...
                    {(casperConnected && (
                      <>
                        <div data-bs-dismiss="modal" id="wallet-connect-casper" className="c-list border-b px-3 py-2 d-flex align-items-center">
                            <div className="text-white col-12 m-auto"> <span style={{wordBreak: 'break-all'}}>{casperAddress}</span></div>
                        </div>
                        <div data-bs-dismiss="modal" id="wallet-connect-casper" className="c-list border-b px-3 py-2 d-flex align-items-center cursor-pointer">
                            <a href={"https://testnet.cspr.live/account/" + casperAddress} target="_blank" className="text-white m-auto" rel="noreferrer"> View on CSPR </a>
                            <a className="text-white m-auto" onClick={ handleDisconnectWalletCasper }> Disconnect</a>
                        </div>
                      </>
                    )) || (
                      <div data-bs-dismiss="modal" id="wallet-connect-casper" className="c-list border-b px-3 py-2 d-flex align-items-center cursor-pointer" onClick={ handleConnectWalletCasper }>
                        <div className="text-white mr-auto"> CasperSigner</div>
                        <img src={caspersigner} width="30px" className="me-2" alt="casperpad" />
                      </div>
                    )}
                </div>
            </div>
